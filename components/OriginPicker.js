...
import { PlaneTakeoff, X, Plus, Search } from 'lucide-react';

// Popular airports for quick-select suggestions
const POPULAR = [
  ...
  { code: 'EWR', name: 'Newark Liberty', city: 'New York', country: 'US' },
...
export default function OriginPicker({ value = [], onChange }) {
  ...
  const results = query.length >= 1 ? filterAirports(query) : POPULAR.slice(0, 6);
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative' }}>
      ...
      <div style={{
        ...
        flexWrap: 'wrap',
        gap: 6,
        marginBottom: value.length > 0 ? 8 : 0,
      }}>
        {value.map(airport => (
          <div key={airport.code} style={{
            ...
            <button
              onClick={() => removeOrigin(airport.code)}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#fff', opacity: .8, padding: 0, display: 'flex', marginLeft: 2 }}
            >
              <X size={13} />
            </button>
          </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  padding: '10px 14px', border: 'none', background: selected ? 'var(--blue-light)' : 'transparent',
              ...
              >
                <div style={{
                  width: 38, height: 28, background: selected ? 'var(--blue)' : 'var(--bg-2)',
                  borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 11, fontWeight: 800, color: selected ? '#fff' : 'var(--blue)',
