...
function buildPrompt(h) {
  ...
  const { destCode = '?', departureDate = '?', currentPrice = 0, daysUntilDeparture = 0, season = 'Unknown', trendDirection = '→', trendPct = 0, historyPoints = 0 } = context;
...
Booking window signal: ${signals.bookingWindow?.signal || recommendation} — ${signals.bookingWindow?.reason || ''}
